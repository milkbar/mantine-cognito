...
export const presign = (config: CognitoConfig, url: string, service: string) => {
    return authenticatedCall<string>(config, (cognitoUser, resolve, reject) => {
        if (!config.identityPoolID) return reject(new Error("identityPoolID is required to presign."))
        const session = cognitoUser.getSignInUserSession()
        if (!session) return reject(new Error("Could not get user session."))
